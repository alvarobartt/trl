...
class ORPOTrainer(Trainer):
    ...
    def tokenize_row(self, feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None) -> Dict:
        ...
        if not self.is_encoder_decoder:
            ...
            def merge_strings(prompt_with_generation_prompt: str, chosen_or_rejected: str) -> str:
                # Find the length of the common suffix/prefix
                common_length = 0
                for i in range(1, min(len(prompt_with_generation_prompt), len(chosen_or_rejected)) + 1):
                    if prompt_with_generation_prompt[-i:] == chosen_or_rejected[:i]:
                        common_length = i
                # Merge the strings based on the common suffix/prefix
                return prompt_with_generation_prompt + chosen_or_rejected[common_length:]

            if not isinstance(chosen, str):
                raise ValueError(f"chosen should be an str but got {type(chosen)}")
            chosen_string = merge_strings(prompt, chosen)
            ...
            rejected_string = merge_strings(prompt, rejected)
